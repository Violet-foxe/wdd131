...
function updateNotesField() {
    ...
    // Show the travel notes on the form if they are 
    // choosing many campuses and require it
    if (value === "many") {
...
function getSelectedCampuses() {
    //.from converts a NodeList into a real array, 
    // so then you can use .filter and .map
...
form.addEventListener("submit", function (event) {
    ...
    // Let the user know if they choose many campuses 
    // but didn't put a note that they need to add a note
    if (type === "many" && notes.length == 0){
        output.textContent = "Please write a Travel Note."
        return;
    }

    //Let the user know if they choose many campus 
    // but only had one campus selected that they need to choose 
    // at least two campuses
    if (type === "many" && selectedCampuses.length < 2){
        output.textContent = "Please choose at least 2 campuses."
        return;
    }
